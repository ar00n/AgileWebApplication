...
export async function register (username, name, password) {
  try {
    ...
    if (e.code === 'SQLITE_CONSTRAINT') { return logEvent({ success: false, action: 'register', message: 'Username already exists.' }, username) }
...
async function createSession (username) {
  ...
  cookies().set('session_username', username, { expires })
  cookies().set('session_token', token, { expires })
